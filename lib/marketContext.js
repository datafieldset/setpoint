...
import { marketBias, rsi } from "./signals.js";
...
// Real, direct finding (Sep 8), tested against real, deep history (up to
// 120 real days, thousands of real events): a whale buy only shows a
// genuine edge when the market is actually oversold at that exact
// moment (57%, holding consistently across two separate, independent
// depth checks) — blended across every condition, or even just
// "ranging," the edge mostly washes out. The mirror, a whale sell
// landing in a genuinely overbought market, showed the same real,
// direct pattern in reverse (55% on a real, if more modest, sample).
// Replaces the old, simple true/false outflow check with the real RSI
// state at the moment of the actual event, computed from these same
// 1-minute candles, not the slower, 4h basis the signal itself fires
// on, since that's genuinely what was tested.
export async function getRecentWhaleActivity() {
  try {
    ...
    if (!r.ok) return { outflowOversold: false, inflowOverbought: false };
    const raw = await r.json();
    if (!Array.isArray(raw) || raw.length < 80) return { outflowOversold: false, inflowOverbought: false };
    ...
    let outflowOversold = false, inflowOverbought = false;
    for (let i = 20; i < candles.length; i++) {
      ...
      const closesSoFar = candles.slice(Math.max(0, i - 60), i + 1).map((x) => x.close);
      const rsiVal = rsi(closesSoFar, 14);
      if (rsiVal == null) continue;
      if (c.close >= c.open && rsiVal < 30) outflowOversold = true;
      if (c.close < c.open && rsiVal > 70) inflowOverbought = true;
    }
    return { outflowOversold, inflowOverbought };
  } catch {
    return { outflowOversold: false, inflowOverbought: false };
  }
}
