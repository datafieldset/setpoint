...
async function computeWeekly200MA() {
  ...
  // static "here's where things stand right now" reading.
...
export async function getWeekly200MA() {
  ...
  try {
    ...
    const fresh = rows.length && rows[0].value?.daily50 != null && rows[0].value?.dailySeries?.length > 0 && rows[0].value?.priceSeries?.length > 0 && (Date.now() - new Date(rows[0].updated_at).getTime()) < MA_CACHE_HOURS * 3600 * 1000;
