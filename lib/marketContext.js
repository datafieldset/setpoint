...
async function computeWeekly200MA() {
  ...
  return { sma, ema, weeksUsed: window.length, daily50, daily200, computedAt: Date.now() };
}
...
export async function getWeekly200MA() {
  ...
  try {
    ...
    const fresh = rows.length && rows[0].value?.daily50 != null && (Date.now() - new Date(rows[0].updated_at).getTime()) < MA_CACHE_HOURS * 3600 * 1000;
