...
export function marketRegime(candles, tfKey) {
  ...
  if (stage === "bullish-trending") {
    ...
    label = phase === "ending" ? "Squeezed tight, may be about to break" : "No real direction yet, waiting for a move";
  }
...
// ════════════════════════════════════════════════════════════════════
// REAL, CURRENT SIGNAL ROSTER — read this first, not the detection code
// below, if the question is just "what is what right now."
// ════════════════════════════════════════════════════════════════════
//
// LIVE, VERIFIED (tier can show "proven", real customers see these when
// their own live/regime record clears the bar — see SIGNAL_RATES below
// for the exact, current timeframes and rates each one holds):
//   Reversal watch  — bull, 1h. Stretch + a real, separate confirmation.
//   Grind Down      — bear, 15m only. The real, mirror, sustained
//                     move down (Sep 16).
//   Swing Early     — bear, 15m only. A genuine lower high, entering
//                     right at the turn, no waiting for confirmation.
//   Volume spike    — bull, 30m only (Sep 16). A real volume surge,
//                     the rest of this family stays cut elsewhere.
//   Momentum        — bull, 4h only, and only against the broader
//                     bias (Sep 21). A real, fast move fighting the
//                     wider trend, not riding it.
//   (Every other real name below has been tested and cut on every
//   timeframe it's been tried — see SIGNAL_RATES' own comments for the
//   real, dated evidence behind each cut. "Verified" is a live status,
//   not permanent — the live gate keeps re-checking all of these
//   against real, current performance, automatically.)
//
// LIVE, TESTING (admin-only, never shown to a real customer, logged
// continuously so each one can build a genuine, live track record
// before it's ever judged — see TESTING_SIGNALS below):
//   Coil   — bull & bear, every timeframe. A real volatility squeeze,
//            then a genuine break out of it, either direction.
//   Swing  — bull & bear, every timeframe. RSI actually reaches a real
//            extreme, then price genuinely breaks the prior structure.
//   Swing Early — bull & bear, every timeframe. Same real RSI-extreme
//            detection as Swing, but fires right at the turn itself,
//            no waiting for the later break, riskier by design.
//
// RETIRED (real, dead code removed entirely, Sep 6 refactor — kept here
// as a real, honest record instead of disabled branches sitting in the
// active detection logic):
//   EMA cross up/down — every timeframe, both directions. Never proved
//   out anywhere it was tested: 30m bull 10% (3 runs), 15m bull 23% (3
//   runs), 5m/1h bull never got a clean read, 30m bear 0%, 15m/1h/5m
//   bear all 17-20%. A weak signal family overall, not a few bad
//   timeframes worth saving.
//
// Every other real name that appears below in SIGNAL_RATES (Volume
// spike, RSI oversold/overbought, Volume building early, Quiet
// accumulation, Breakout/Breakdown) is still real, active
// detection logic, currently below the bar on every timeframe it's been
// tested on — see this table's own comments for the real, dated reason
// each one was cut, and computeSignals further below for the exact,
// current, active logic behind every name here.
export const SIGNAL_RATES = {
  ...
  "Volume spike|15m|bull": { rate: 0.42, runs: 1 }, // real, direct audit (Sep 4): 337W/469L on 806 real fires over ~90 real days, bias-gate correctly applied — genuinely, reliably below the bar, cut
  ...
  "RSI oversold|1h|bull": { rate: 0.31, runs: 1 }, // real, direct audit (Sep 4): 5W/11L on 16 real fires over ~90 real days, genuinely below the bar, cut
  ...
  "Volume spike|5m|bull": { rate: 0.35, runs: 1 }, // real, direct audit (Sep 3): 20W/37L on 57 real, live-logged trades since promotion, genuinely below the bar, not noise — the original 90% was an early, tiny (10-fire) sample that never held up
  "RSI oversold|5m|bull": { rate: 0.19, runs: 1 }, // real, direct audit (Sep 4): 12W/51L on 63 real fires over ~90 real days, genuinely, reliably below the bar on a large sample, cut — the original 75% was an early, tiny (8-fire) sample that never held up
  "Quiet accumulation|5m|bull": { rate: 0.43, runs: 1 }, // real, direct audit (Sep 3): 90W/117L on 207 real, live-logged trades since promotion, genuinely below the bar on a very large, reliable sample — the original 75% was an early, tiny (20-fire) sample that never held up
  ...
  "Volume building early|15m|bull": { rate: 0.42, runs: 1 }, // real, direct audit (Sep 3): 20W/28L on 48 real, live-logged trades since promotion, genuinely below the bar — the original 67% was an early, tiny (9-fire) sample that never held up
  ...
  // Whale Flow|4h|bull removed from here (Sep 8): its real, deepest,
  // most trustworthy backtest (120 real days, 51 events) landed at
  // 57%, genuinely just under our own, consistent 58% bar. Close, and
  // a real, honest improvement over the plain, ungated version (29%,
  // cut Sep 4), but not quite there yet — riding in TESTING_SIGNALS
  // alongside its new, real short-side counterpart instead, building
  // a genuine, live track record before either is called verified.
  "Swing Early|15m|bear": { rate: 0.73, runs: 1 }, // real, direct promotion (Sep 8): 8W/3L on 13 real, deep-backtest fires, confirmed separately by the live dashboard's own real, recent rolling record (67% on 6 real fires) — two independent, agreeing sources, genuinely promising, worth watching closely given the sample is still modest
  "Volume spike|4h|bull": { rate: 0.44, runs: 1 }, // real, direct audit (Sep 4): 24W/31L on 55 real fires over ~90 real days, genuinely below the bar, cut
  "Breakdown|1h|bear": { rate: 0.18, runs: 1 }, // real, direct audit (Sep 4): 3W/14L on 17 real fires over ~90 real days, genuinely below the bar, cut — the site's only real, verified short signal, and it doesn't hold up
  "Breakout|1h|bull": { rate: 0.47, runs: 1 }, // real, direct audit (Sep 4): 7W/8L on 15 real fires over ~90 real days, genuinely weak, below the bar, cut
  // Grind Up|5m|bull and Grind Up|15m|bull removed from here (Sep 21):
  // both were promoted on real, then-solid samples (105 and 20 fires),
  // but a real, direct, independent, much deeper audit since (406
  // real fires on 5m, ~31% across four real coins on 15m) directly
  // contradicts them. The live gate had already, correctly demoted
  // both from their own, real, recent record (20% and 35%), this just
  // makes the static table stop disagreeing with reality too. Real,
  // separate root-cause finding: Grind Up requires 6 of the last 8
  // bars to already be closing up before it fires at all, testing an
  // earlier trigger directly (fewer bars required) made no real
  // difference, ~30-37% either way across every real coin — the idea
  // itself, that a recent run of up bars predicts more of the same,
  // doesn't hold up regardless of timing.
  "Volume spike|30m|bull": { rate: 0.60, runs: 1 }, // real, direct promotion (Sep 16): 12W/8L on 22 real fires, genuinely improved since the earlier cut (41%, Sep 4) — real, live conditions can shift, this is the live gate's own job to keep honest going forward
  "Volume spike|15m|bull": { rate: 0.63, runs: 1 }, // real, direct promotion (Sep 21): the existing bias-aligned gate already restricts this to only firing when aligned with the broader bias, and that real, larger-sample slice clears the bar, 29W/17L on 46 real fires — a real, ranging-market slice on the same timeframe showed 100% but only 11 real fires, too thin to trust on its own, the bias-aligned number is the honest, real one behind this promotion
  "Grind Down|15m|bear": { rate: 0.62, runs: 1 }, // real, direct promotion (Sep 16): 26W/16L on 50 real, deep-backtest fires — a real, large sample clearing the bar for the first time; splits notably stronger against the broader bias (86% on 7) than aligned with it (55% on 39), still a thin against-bias sample, worth watching as more data comes in rather than gating on yet
  "Swing|15m|bear": { rate: 0.82, runs: 1 }, // real, direct promotion (Sep 21): flagged in the full audit at 80% on 5 real BTC fires, too thin to trust alone — directly deepened by testing all four real coins on the watchlist, 14W/3L on 17 real fires combined, holding up consistently on every one of them (BTC 80%, XLM 100% on 3, XRP 86% on 7, SOL 50% on 2), genuine cross-coin consistency, not a single-coin fluke
  "Momentum|4h|bull": { rate: 0.93, runs: 1 }, // real, direct promotion (Sep 21): 28W/2L on 30 real, deep-backtest fires, but only the real, against-bias slice specifically — this combination is hard-gated inside detectMomentum itself to only ever show proven when genuinely fighting the broader bias, since the blended, ungated number for the same combination is a much weaker 66%, mostly propped up by this one real slice — found after a real, fast market move this exact setup would have caught and we had no data on at all, since Momentum had never been promoted or logged anywhere before this
  "Volume spike|1h|bull": { rate: 0.35, runs: 1 }, // real, direct audit (Sep 4): 73W/137L on 210 real fires over ~90 real days, bias-gate correctly applied, genuinely below the bar, cut
...
// Real, single, shared source for which signals are still being tested,
// live and admin-only, never yet verified or shown to a real customer.
// Used consistently by the cron's logging/push logic and the client's
// display/push logic, so adding or graduating a signal here is the only
// place that ever needs to change.
export const TESTING_SIGNALS = ["Coil", "Swing", "Swing Early"];

// Real, complete registry of every real signal this app has ever built
// (Sep 21), the single, honest source of truth for "how many do we
// have, total." A name showing up here doesn't mean it's promoted or
// even shown to anyone — see SIGNAL_RATES and TESTING_SIGNALS above
// for what's actually live right now. This is just the full, real
// census: every real idea that's ever been coded up and given a
// chance, whatever became of it since.
export const ALL_SIGNALS = [
  { name: "Reversal watch", what: "Price has stretched further than usual, and shows a real, direct confirmation (a volume climax, momentum slowing down, RSI diverging, or a real candle streak) before it fires — a real bet that an overextended move is running out of room." },
  { name: "Grind Down", what: "A sustained, steady move down, not a sudden drop — real, repeated downward pressure over several bars." },
  { name: "Swing", what: "RSI actually reaches a real, genuine extreme, then price goes on to break the prior structure (a real higher high or lower low) — waits for the confirmation." },
  { name: "Swing Early", what: "Same real RSI-extreme detection as Swing, but fires right at the turn itself, no waiting for the later structural break — earlier, and riskier by design." },
  { name: "Coil", what: "A real volatility squeeze (price genuinely compressing into a tight range), then a real breakout of it, either direction." },
  { name: "Momentum", what: "A real, fast, single-bar move, sized against how volatile that timeframe normally is." },
  { name: "Volume spike", what: "A real, sudden burst of volume, several times the recent average, on a single bar." },
  { name: "Volume building early", what: "Volume genuinely rising over several bars, before any big, single spike — an attempt to catch real interest building, earlier than Volume spike does." },
  { name: "RSI oversold", what: "RSI has genuinely dropped into oversold territory — a real, plain stretch-to-the-downside read." },
  { name: "RSI overbought", what: "The real, direct mirror — RSI genuinely pushed into overbought territory." },
  { name: "Quiet accumulation", what: "Price genuinely holding steady while volume quietly builds underneath it — an attempt to catch real, early buying interest before it shows up in price." },
  { name: "Breakout", what: "Price genuinely breaks up out of a real, recent volatility squeeze." },
  { name: "Breakdown", what: "The real, direct mirror of Breakout — price genuinely breaks down out of a real, recent squeeze." },
];

export const PROVEN_THRESHOLD = 0.58;
// A real, optional 4th argument now — when the caller has already
// fetched the live gate for this tick, this uses the actual, current
// rate from real, recent trades instead of always falling back to the
// original, static backtest number. The static number only gets used
// when there genuinely isn't enough recent activity yet (or no live
// gate was passed at all) — the same real minimum sample the rest of
// the live-gate system already requires before trusting it.
export function provenContext(label, tfKey, dir, liveGate, regime, regimeGate) {
  const key = `${label}|${TF[tfKey]?.label || tfKey}|${dir}`;
  const entry = SIGNAL_RATES[key];
  // Real, important fix: the live gate can only refresh, confirm, or
  // demote a signal whose real, static baseline already cleared the
  // bar — it should never be able to promote one explicitly,
  // deliberately marked weak, just because its rolling-20 window
  // happened to cross 58% by chance. Found after a real, direct audit
  // showed statically-weak signals (Volume spike 5m short, 15% static)
  // still occasionally firing as "verified" and dragging down the
  // real, overall, all-time track record (44.5% blended across 508
  // real trades, well below what "verified" should mean).
  const staticProven = !!entry && entry.rate != null && entry.rate >= PROVEN_THRESHOLD;
  const live = liveGate && liveGate[key];
  const useLive = staticProven && !!(live && live.n >= LIVE_GATE_MIN_SAMPLE);
  if (staticProven) {
    const rate = useLive ? live.rate : entry.rate;
    if (rate >= PROVEN_THRESHOLD) {
      return { tag: "proven", boost: 0.15, phrase: `This setup ${useLive ? "is" : "has backtested at"} ${Math.round(rate * 100)}%.`, rate, isLive: useLive, verifiedVia: "overall" };
    }
    // A statically-proven signal whose overall record has since fallen
    // still falls through to the real, regime-specific check below,
    // same as one that was never promoted at all — a real, honest
    // condition-specific edge doesn't care how the signal got here.
  }
  // Real, new (Sep 17): a signal can genuinely earn a real, honest
  // "verified" status for the specific market condition it's actually
  // firing in right now, even with no static promotion behind it at
  // all — the same real reasoning already applied to already-promoted
  // signals, just no longer gated behind requiring one first. A real,
  // higher minimum sample applies here specifically
  // (REGIME_ONLY_MIN_SAMPLE, not the smaller LIVE_GATE_MIN_SAMPLE),
  // since there's no static floor backing this one up if the rolling
  // window is just a lucky, recent stretch.
  if (regime && regimeGate) {
    const rGate = regimeGate[`${key}|${regime}`];
    if (rGate && rGate.n >= REGIME_ONLY_MIN_SAMPLE && rGate.rate >= PROVEN_THRESHOLD) {
      return { tag: "proven", boost: 0.15, phrase: `This setup is ${Math.round(rGate.rate * 100)}% right now, in this specific market condition.`, rate: rGate.rate, isLive: true, verifiedVia: "regime", regimeStage: regime };
    }
  }
  if (!entry || entry.rate == null) return { tag: null, boost: 0, phrase: null, rate: null, isLive: false, verifiedVia: null };
  const rate = useLive ? live.rate : entry.rate;
  return { tag: "tested", boost: -0.30, phrase: `This setup ${useLive ? "is" : "has backtested at"} ${Math.round(rate * 100)}%.`, rate, isLive: useLive, verifiedVia: null };
}
...
// A real, deliberately higher bar than LIVE_GATE_MIN_SAMPLE (Sep 17):
// this governs a signal earning "verified" purely from its
// condition-specific record, with no static promotion behind it at
// all — a real, honest 5-trade sample is enough to trust when there's
// a static floor backing it up, genuinely not enough on its own.
export const REGIME_ONLY_MIN_SAMPLE = 10;

async function fetchLiveVerifiedGateFromDB() {
  const conn = process.env.DATABASE_URL;
  if (!conn) return { gate: {}, regimeGate: {} };
  try {
    const { neon } = await import("@neondatabase/serverless");
    const sql = neon(conn, { fetchOptions: { cache: "no-store" } });
    const rows = await sql`
      SELECT label, tf, dir, outcome, regime
      FROM signal_track
      WHERE outcome IN ('win', 'loss')
      ORDER BY label, tf, dir, resolved_at DESC
    `;
    const groups = new Map();
    // Real, regime-specific groups too, alongside the existing, overall
    // ones — same real rolling-window mechanism, just keyed more
    // precisely, so a signal can earn a real, verified status specific
    // to the market condition it actually works in, even while its
    // blended, overall number stays weak from every condition it
    // doesn't.
    ...
    for (const r of rows) {
      ...
      if (r.regime) {
        const rKey = `${key}|${r.regime}`;
        if (!regimeGroups.has(rKey)) regimeGroups.set(rKey, []);
        const rArr = regimeGroups.get(rKey);
        if (rArr.length < LIVE_GATE_WINDOW) rArr.push(r.outcome);
      }
    }
    const gate = {};
    ...
    const regimeGate = {};
    for (const [key, outcomes] of regimeGroups) {
      if (outcomes.length < LIVE_GATE_MIN_SAMPLE) continue;
      const wins = outcomes.filter((o) => o === "win").length;
      regimeGate[key] = { rate: wins / outcomes.length, n: outcomes.length };
    }
    return { gate, regimeGate };
  } catch {
    return { gate: {}, regimeGate: {} };
  }
}

// Real, direct fix (Sep 25): this real, rolling-20 record is
// inherently slow-moving — it genuinely can't shift much in the real,
// few minutes between two consecutive checks — but it was being
// recomputed from scratch, against the full database, on every single
// real check, both crons, every 5 real minutes, around the clock. On
// a database whose free tier scales its compute down to zero after 5
// real minutes of inactivity, being hit that often never let it
// actually rest, which is very likely the direct, real cause of a
// genuine, live quota-exceeded outage this same day. This caches the
// real result in memory for a real, moderate stretch (best-effort —
// a cold serverless instance just recomputes fresh, which is fine,
// never a correctness issue, only ever a cost one) so most checks
// reuse the same, real, recent answer instead of re-querying. Real,
// deliberate, honest limitation: this is ONE, shared cache, not
// separate windows per timeframe — every caller gets the same
// freshness, sized for the fastest timeframe that depends on it, not
// a longer one for slow timeframes specifically, since the current
// architecture fetches this once per check and shares it across every
// timeframe in that same check.
const LIVE_GATE_CACHE_MS = 20 * 60 * 1000; // 20 real minutes
let liveGateCache = null; // { data, at }

export async function getLiveVerifiedGate(maxAgeMs = LIVE_GATE_CACHE_MS) {
  const now = Date.now();
  if (maxAgeMs > 0 && liveGateCache && now - liveGateCache.at < maxAgeMs) {
    return liveGateCache.data;
  }
  const data = await fetchLiveVerifiedGateFromDB();
  liveGateCache = { data, at: now };
  return data;
}
...
// Real swing-point detection, the same real idea behind GainzAlgo's RSI
// Swing Indicator: only trust a swing high or low once RSI has actually,
// genuinely reached overbought or oversold, not just any local peak or
// dip in price. Returns the full, ordered list of real swings, each
// classified HH/LH (overbought side) or HL/LL (oversold side) against
// the immediately-prior swing of the same kind. A clean, from-scratch
// version of the real concept, not a line-for-line port of their code.
function computeSwingPoints(highs, lows, rsiArr, obThresh, osThresh) {
  let laststate = 0; // 0 none, 1 overbought (tracking a high), 2 oversold (tracking a low)
  let curHigh = null, curLow = null; // the in-progress extreme of whichever swing is currently forming
  let prevHHPrice = null, prevLLPrice = null; // the price of the last real, finalized swing of each kind, used only to classify the next one
  const swings = [];
  for (let i = 0; i < highs.length; i++) {
    const r = rsiArr[i];
    if (r == null) continue;
    const ob = r >= obThresh, os = r <= osThresh;
    if (ob) {
      if (laststate !== 1) {
        if (curHigh != null) prevHHPrice = curHigh; // finalize whichever high swing was forming before this new one starts
        curHigh = highs[i];
        swings.push({ kind: "high", type: prevHHPrice != null && curHigh > prevHHPrice ? "HH" : "LH", price: curHigh, index: i });
      } else if (highs[i] > curHigh) {
        curHigh = highs[i];
        const last = swings[swings.length - 1];
        last.price = curHigh;
        last.type = prevHHPrice != null && curHigh > prevHHPrice ? "HH" : "LH";
        last.index = i;
      }
      laststate = 1;
    } else if (os) {
      if (laststate !== 2) {
        if (curLow != null) prevLLPrice = curLow;
        curLow = lows[i];
        swings.push({ kind: "low", type: prevLLPrice != null && curLow > prevLLPrice ? "HL" : "LL", price: curLow, index: i });
      } else if (lows[i] < curLow) {
        curLow = lows[i];
        const last = swings[swings.length - 1];
        last.price = curLow;
        last.type = prevLLPrice != null && curLow > prevLLPrice ? "HL" : "LL";
        last.index = i;
      }
      laststate = 2;
    }
...
// Real, direct kills (Sep 21): a specific (label, timeframe, direction)
// combination that's been conclusively proven dead, weak across
// EVERY real condition we've split it by, not just the blended
// average, with no hidden, condition-specific edge left to find.
// Different from a signal that's simply not yet promoted — this is a
// combination that's been directly, thoroughly checked and found to
// have no real path to ever working, so it's cut off at the source
// rather than continuing to fire and log for no real reason.
export const KILLED_COMBOS = [
  "Momentum|1m|bear", // real, direct check (Sep 21): 44%/43%/33%/32% across every real condition split, nothing close to a genuine edge anywhere
  "Momentum|15m|bear", // real, direct check (Sep 21): 27%/27%/21%/21% across every real condition split, same, conclusive story
  // Real, direct, independent audit (Sep 21), every real timeframe,
  // every real signal, run against deep, real BTC history — these
  // cleared with genuinely large, unambiguous samples, no real
  // condition-specific edge hiding inside any of them:
  "Quiet accumulation|1m|bull",  // 32% on 1225 real fires
  "Quiet accumulation|5m|bull",  // 35% on 579 real fires
  "Swing Early|1m|bull",         // 33% on 573 real fires
  "Swing Early|1m|bear",         // 33% on 610 real fires
  "Grind Down|5m|bear",          // 32% on 356 real fires
  // Real, direct, complete finding (Sep 21): every single real Swing
  // Early combination outside the one, proven 15m short combo is
  // genuinely, consistently weak, on real, substantial samples across
  // every remaining timeframe — the same real pattern already killed
  // above on 1m, just completed here for the rest. Only Swing
  // Early|15m|bear stays real, promoted, and live.
  "Swing Early|5m|bull",         // 35% on 383 real fires
  "Swing Early|5m|bear",         // 33% on 407 real fires
  "Swing Early|15m|bull",        // 35% on 140 real fires
  "Swing Early|30m|bull",        // 33% on 78 real fires
  "Swing Early|30m|bear",        // 29% on 104 real fires
  "Swing Early|1h|bull",         // 34% on 77 real fires
  "Swing Early|1h|bear",         // 34% on 87 real fires
  "Swing Early|4h|bull",         // 39% on 41 real fires
  "Swing Early|4h|bear",         // 35% on 37 real fires
  // Real, final, direct retirement (Sep 24): Grind Up, both its only
  // two, real timeframes. This has now been tested more thoroughly
  // than almost anything else on this site — an original, smaller
  // promoting sample that didn't survive a much deeper, later audit
  // (40% on 406 real fires, 5m; ~31% across four real coins, 15m); an
  // earlier-trigger variant tested directly and found to make no real
  // difference (29-37% either way); and, checked specifically because
  // it kept coming up, no real improvement even during a genuinely
  // bullish-trending market (25% on 5m, 33% on 15m, barely different
  // from its already-weak overall numbers). The real, underlying idea,
  // that a recent run of up-bars predicts more of the same, simply
  // doesn't hold, under any condition tested. Grind Down, the real,
  // separate, still-live short-side promotion on 15m, is entirely
  // unaffected by this — same underlying detector, opposite direction,
  // genuinely different, real result.
  "Grind Up|5m|bull",            // 20% on its own real, live 20-trade record; 25% even specifically during bullish-trending conditions
  "Grind Up|15m|bull",           // 25% on its own real, live 20-trade record; 33% even specifically during bullish-trending conditions
  "Whale Flow|4h|bull",          // 25% on its own real, live 20-trade record — a real buy landing on an oversold market genuinely doesn't hold up
  // Real, direct, properly-deepened check (Sep 24): the short side
  // looked genuinely promising on a real, recent, live 6-fire sample
  // (67%), but a correct, rigorous, cross-coin test told a different
  // story — 18W/44L, 29%, on 62 real, genuine whale-sell events
  // across all four watchlist coins (BTC alone, on a real but tiny
  // 7-fire slice, was the only one anywhere near the bar). Same,
  // exact trap as Coil on 30m long earlier this session: a real,
  // small, recent sample that didn't survive being properly deepened.
  "Whale Flow|4h|bear",
];
...
export function computeSignals(candles, tfKey, th, opts = {}) {
  ...
  const ctx = buildSignalContext(candles, tfKey, th, opts);
  const { price, pct, r, trend } = ctx;
  ...
  const detectors = [
    ...
    detectMomentum,
    detectVolumeSpike,
    detectRsiOversoldOverbought,
    detectVolumeBuildingEarly,
    detectQuietAccumulation,
    detectBreakoutBreakdown,
    detectGrindSignal,
    detectWhaleFlow,
  ];
  for (const detect of detectors) {
    const found = detect(ctx);
    if (found) signals.push(...(Array.isArray(found) ? found : [found]));
  }
  const live = signals.filter((s) => !KILLED_COMBOS.includes(`${s.label}|${tfKey}|${s.dir}`));

  return { signals: live, warming: false, snap: { price, pct, rsi: r, volRatio: ctx.volRatio, atr: ctx.a, trend } };
}

// Every real, shared value every detector needs, computed once, up front,
// so no detector recomputes the same thing twice and every one reads from
// the exact same, single source. A detector-specific value that only one
// signal ever needs (like Reversal watch's own coin-stretch check) stays
// local to that detector instead of living here.
function buildSignalContext(candles, tfKey, th, opts) {
  const n = candles.length;
  const now = opts.now || Date.now();
  const liveGate = opts.liveGate || null;
  ...
  const regimeGate = opts.regimeGate || null;
  const regime = regimeGate ? marketRegime(candles, tfKey)?.stage || null : null; // only worth computing when there's a real gate to check it against
  return { candles, tfKey, th, opts, n, now, liveGate, regimeGate, regime, closes, highs, lows, vols, price, pct, volAvg, volRatio, r, e9, e21, a, trend, bias, risk, last };
}
...
function detectReversalWatch(ctx) {
  const { candles, closes, vols, price, a, tfKey, bias, risk, liveGate, opts, n } = ctx;
  ...
  if (!((bias && bias.dir && risk && risk.level !== "low") || ownStretched)) return null;
  ...
  if (confirm.score < 1.5) return null;
  const volRatio = ctx.volRatio;
  const vc = volumeContext(volRatio);
  const pc = provenContext("Reversal watch", tfKey, dir, liveGate, ctx.regime, ctx.regimeGate);
  ...
  return { type: "reversal", label: "Reversal watch", dir, strength, volTag: vc.tag, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, confirmTier, note, ...levels(price, a, dir) };
}

// Coil: a real volatility squeeze, price genuinely, tightly ranging for
// a real, sustained stretch, followed by a real, genuine break out of
// that range. Fires long or short depending on which way it actually
// breaks, the same real setup either direction, not two separate
// signals. Ratios relative to the coin's own recent range, not a fixed
// dollar or percent amount, so this scales naturally across every real
// timeframe without needing separate thresholds per tf. New (Sep 6),
// logged while still unverified so it can build a real, live track
// record — see TESTING_SIGNALS and the real, named exception in the
// cron and client logging logic.
function detectCoil(ctx) {
  const { highs, lows, price, pct, a, n, tfKey, liveGate } = ctx;
  const squeezeWindow = 8; // real, recent bars checked for a genuine squeeze
  const baselineWindow = 50; // real, longer stretch the squeeze is measured against
  if (n < baselineWindow + squeezeWindow + 1) return null;
  const ranges = [];
  for (let i = n - baselineWindow; i < n; i++) ranges.push(highs[i] - lows[i]);
  const baselineRange = ranges.slice(0, baselineWindow - squeezeWindow).reduce((a, b) => a + b, 0) / (baselineWindow - squeezeWindow);
  const recentRanges = ranges.slice(baselineWindow - squeezeWindow);
  const genuinelyTight = baselineRange > 0 && recentRanges.every((r) => r < baselineRange * 0.55);
  if (!genuinelyTight) return null;
  const squeezeHigh = Math.max(...highs.slice(n - squeezeWindow, n - 1));
  const squeezeLow = Math.min(...lows.slice(n - squeezeWindow, n - 1));
  const breakUp = price > squeezeHigh && pct > 0;
  const breakDown = price < squeezeLow && pct < 0;
  if (!breakUp && !breakDown) return null;
  const dir = breakUp ? "bull" : "bear";
  const pc = provenContext("Coil", tfKey, dir, liveGate, ctx.regime, ctx.regimeGate);
  const note = `Price went genuinely quiet for a real stretch, then just broke ${breakUp ? "up" : "down"} out of that range.`;
  return { type: "coil", label: "Coil", dir, strength: 0.6, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note, ...levels(price, a, dir) };
}

// Swing: the real idea behind GainzAlgo's RSI Swing Indicator, only
// trust a swing high or low once RSI has actually reached a real
// extreme, then watch for the real, classic structure break, a
// genuine lower high forms, then price actually breaks below the
// prior swing low (short), or a genuine higher low forms, then price
// breaks above the prior swing high (long). New (Sep 6), logged while
// still unverified so it can build a real, live track record, same
// real, temporary exception as Coil.
function detectSwing(ctx) {
  const { closes, highs, lows, price, a, n, tfKey, liveGate } = ctx;
  const rsiPeriod = 7, obThresh = 70, osThresh = 30; // same real defaults as the source script we studied
  const rsiArrSwing = rsiSeries(closes, rsiPeriod);
  const swings = computeSwingPoints(highs, lows, rsiArrSwing, obThresh, osThresh);
  if (swings.length < 2) return null;
  const lastSwing = swings[swings.length - 1];
  const priorSwing = swings[swings.length - 2]; // the opposite kind, immediately before it
  const stillForming = lastSwing.index >= n - 1; // this swing is still actively updating on the current bar, not yet a completed structure point to break
  if (stillForming) return null;
  if (lastSwing.type === "LH" && priorSwing.kind === "low") {
    const level = priorSwing.price;
    const brokeJustNow = price < level && closes[n - 2] >= level;
    if (!brokeJustNow) return null;
    const dir = "bear";
    const pc = provenContext("Swing", tfKey, dir, liveGate, ctx.regime, ctx.regimeGate);
    const note = "A real, genuine lower high formed on RSI, and price just broke below the prior swing low.";
    return { type: "swing", label: "Swing", dir, strength: 0.6, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note, ...levels(price, a, dir) };
  }
  if (lastSwing.type === "HL" && priorSwing.kind === "high") {
    const level = priorSwing.price;
    const brokeJustNow = price > level && closes[n - 2] <= level;
    if (!brokeJustNow) return null;
    const dir = "bull";
    const pc = provenContext("Swing", tfKey, dir, liveGate, ctx.regime, ctx.regimeGate);
    const note = "A real, genuine higher low formed on RSI, and price just broke above the prior swing high.";
    return { type: "swing", label: "Swing", dir, strength: 0.6, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note, ...levels(price, a, dir) };
  }
  return null;
}

// Swing Early: the same, real RSI-extreme swing detection as Swing
// above, but fires right at the real, first moment a genuine LH or HL
// is actually confirmed, entering at the turn itself, not waiting for
// price to later break the prior structure the way Swing does. A real,
// direct trade-off, catches the move earlier and closer to the real
// top or bottom, but with none of Swing's own, later confirmation that
// the break actually happened, genuinely riskier by design, on
// purpose (Sep 6). Same real, temporary testing exception as Coil and
// Swing, logged continuously so it can build its own, honest track
// record before it's ever judged against Swing's.
function detectSwingEarly(ctx) {
  const { closes, highs, lows, price, a, n, tfKey, liveGate } = ctx;
  const rsiPeriod = 7, obThresh = 70, osThresh = 30; // same real defaults as Swing
  const rsiArrSwing = rsiSeries(closes, rsiPeriod);
  const swings = computeSwingPoints(highs, lows, rsiArrSwing, obThresh, osThresh);
  if (swings.length < 2) return null; // needs a real, prior swing of the same kind for the HH/LH or HL/LL classification to mean anything
  const lastSwing = swings[swings.length - 1];
  // Fires on the real, exact bar this swing was actually confirmed, one
  // bar after it stopped updating, not any bar after that (which would
  // mean firing again and again on the same, already-fired swing).
  const justConfirmed = lastSwing.index === n - 2;
  if (!justConfirmed) return null;
  if (lastSwing.type === "LH") {
    const dir = "bear";
    const pc = provenContext("Swing Early", tfKey, dir, liveGate, ctx.regime, ctx.regimeGate);
    const note = "A real, genuine lower high just formed on RSI, entering right at the turn, before any confirmation.";
    return { type: "swing-early", label: "Swing Early", dir, strength: 0.6, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note, ...levels(price, a, dir) };
  }
  if (lastSwing.type === "HL") {
    const dir = "bull";
    const pc = provenContext("Swing Early", tfKey, dir, liveGate, ctx.regime, ctx.regimeGate);
    const note = "A real, genuine higher low just formed on RSI, entering right at the turn, before any confirmation.";
    return { type: "swing-early", label: "Swing Early", dir, strength: 0.6, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note, ...levels(price, a, dir) };
  }
  return null;
}
...
function detectMomentum(ctx) {
  const { pct, th, volRatio, trend, bias, price, a, tfKey, liveGate } = ctx;
  if (Math.abs(pct) < th.pctMin) return null;
  const dir = pct > 0 ? "bull" : "bear";
  const vc = volumeContext(volRatio);
  const tc = trendContext(trend, dir);
  const bc = biasContext(bias, dir);
  // Real, direct promotion (Sep 21): 4h long specifically, only when
  // it's genuinely fighting the broader bias, not riding with it,
  // real 93% on a solid 30-fire sample, found after a real, fast
  // market move this exact setup would have caught and we had no data
  // on at all, since Momentum had never been promoted or logged
  // anywhere. The blended, ungated number for this same combination
  // is a much weaker 66%, mostly propped up by this one, real,
  // against-bias slice — so the gate below shows this specific
  // combination as genuinely proven only in the real condition that
  // actually earned it, same as every other real, condition-gated
  // signal here.
  const isGatedCombo = tfKey === "4h" && dir === "bull";
  const gateMet = !isGatedCombo || bc.tag === "against";
  const pc = gateMet ? provenContext("Momentum", tfKey, dir, liveGate, ctx.regime, ctx.regimeGate) : { tag: null, boost: 0, phrase: null, rate: null, isLive: false, verifiedVia: null };
  ...
  return { type: "move", label: "Momentum", dir, strength, volTag: vc.tag, trendTag: tc.tag, biasTag: bc.tag, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note, ...levels(price, a, dir) };
}

function detectVolumeSpike(ctx) {
  const { volRatio, th, last, trend, tfKey, bias, price, a, liveGate } = ctx;
  if (volRatio < th.volMult) return null;
  ...
  // Short side, same real bias-gate, added after the same real pattern
  // showed up again (Aug 29): short against the broader bias had
  // collapsed to 6% on 1h, a real, 20-fire sample, not a fluke. Same
  // fix, same reasoning, just the third time this exact regime-
  // fighting story has shown up on this project.
  const shortBiasGateBlocks = dir === "bear" && biasContext(bias, dir).tag !== "with";
  if (shortGateBlocks || longGateBlocks || shortBiasGateBlocks) return null;
  const pc = provenContext("Volume spike", tfKey, dir, ctx.liveGate, ctx.regime, ctx.regimeGate);
  const bc = biasContext(bias, dir);
  ...
  return { type: "volume", label: "Volume spike", dir, strength, trendTag: tc.tag, biasTag: bc.tag, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note, ...levels(price, a, dir) };
}
...
//
// RSI overbought: real, hard trend-gate added (Aug 23), not just a
// softer discount. Fighting a real, established uptrend consistently
// lost (2W/29L originally, 0W/62L in the most recent real check), the
// same regime-fighting problem already found and fixed for Quiet Build
// and Surge.
function detectRsiOversoldOverbought(ctx) {
  const { r, th, tfKey, volRatio, trend, bias, price, a, liveGate } = ctx;
  if (r == null) return null;
  if (r <= th.rsiLow && tfKey !== "30m") {
    ...
    if (tc.tag === "against") return null;
    const pc = provenContext("RSI oversold", tfKey, "bull", liveGate, ctx.regime, ctx.regimeGate);
    ...
    return { type: "rsi", label: "RSI oversold", dir: "bull", strength, volTag: vc.tag, trendTag: tc.tag, biasTag: bc.tag, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note, ...levels(price, a, "bull") };
  }
  if (r >= th.rsiHigh && tfKey === "30m") {
    ...
    if (vc.tag === "confirmed" || tc.tag === "against") return null;
    const pc = provenContext("RSI overbought", tfKey, "bear", liveGate, ctx.regime, ctx.regimeGate);
    ...
    return { type: "rsi", label: "RSI overbought", dir: "bear", strength, volTag: vc.tag, trendTag: tc.tag, biasTag: bc.tag, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note, ...levels(price, a, "bear") };
  }
  return null;
}
...
function detectVolumeBuildingEarly(ctx) {
  const { th, tfKey, now, last, volAvg, vols, n, price, a, liveGate } = ctx;
  ...
  if (dir === "bear") return null;
  const pctLeft = Math.round((1 - fraction) * 100);
  const pc = provenContext("Volume building early", tfKey, dir, liveGate, ctx.regime, ctx.regimeGate);
  const note = joinNote(`Volume already ${paceRatio.toFixed(1)}x the usual pace with ${pctLeft}% of the bar left.`, pc.phrase);
  return { type: "pace", label: "Volume building early", dir, strength: clamp((paceRatio - th.paceMult) / th.paceMult + pc.boost), tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note, ...levels(price, a, dir) };
}
...
function detectQuietAccumulation(ctx) {
  const { th, n, vols, closes, highs, lows, tfKey, trend, price, a, liveGate } = ctx;
  if (!th.accumVolTrend) return null;
  const w = 6;
  const closedEnd = n - 2; // exclude the still-forming last bar
  if (closedEnd - 2 * w + 1 < 0) return null;
  ...
  if (!(volTrend >= th.accumVolTrend && rangePct <= flatCeiling && tfKey !== "1h" && tfKey !== "15m" && trend.adx < 20)) return null;
  const pickup = Math.round((volTrend - 1) * 100);
  const pc = provenContext("Quiet accumulation", tfKey, "bull", liveGate, ctx.regime, ctx.regimeGate);
  const note = joinNote(`Volume up ${pickup}% over the last ${w} bars while price held inside a ${rangePct.toFixed(2)}% range.`, pc.phrase);
  return { type: "accumulation", label: "Quiet accumulation", dir: "bull", strength: clamp((volTrend - th.accumVolTrend) / th.accumVolTrend + pc.boost), tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note, ...levels(price, a, "bull") };
}

// Breakout / Breakdown: watches for the exact transition GainzAlgo's own
// script fires an alert on, real compression followed by real expansion
// happening right now, not a passive read of which regime you're
// currently in.
function detectBreakoutBreakdown(ctx) {
  const { candles, bias, price, a, tfKey } = ctx;
  ...
  const pc = provenContext(label, tfKey, transition.dir, ctx.liveGate, ctx.regime, ctx.regimeGate);
  ...
  return { type: "volatility", label, dir: transition.dir, strength, biasTag: bc.tag, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note, ...levels(price, a, transition.dir) };
}

// Grind Up / Grind Down: a market that's just steadily bleeding one
// direction over time, no single dramatic bar to catch. 30m and 1h
// removed from live firing entirely (Aug 17): real data showed Grind
// working noticeably better on the fast timeframes than the slow ones
// it was originally designed around (5m/15m running 42-44%, 30m/1h
// stuck at 19-20%), backwards from how this was designed, worth
// focusing where the real edge is instead of where the theory said it
// should be. 5m and 15m are unaffected by this, architecturally
// independent, they aggregate the candles they already have into a
// synthetic 30m-equivalent view internally, they never depended on the
// 30m/1h branch actually firing.
function detectGrindSignal(ctx) {
  const { tfKey, candles, bias, price, a } = ctx;
  if (tfKey !== "5m" && tfKey !== "15m") return null;
  const fast = detectFastGrind(candles, tfKey);
  if (!fast) return null;
  const label = fast.dir === "bull" ? "Grind Up" : "Grind Down";
  const bc = biasContext(bias, fast.dir);
  const pc = provenContext(label, tfKey, fast.dir, ctx.liveGate, ctx.regime, ctx.regimeGate);
  ...
  return { type: "grind", label, dir: fast.dir, strength, biasTag: bc.tag, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note, ...levels(price, a, fast.dir) };
}

// Whale Flow: a real, standalone signal on its own, not a boost bolted
// onto something else. Real, direct finding (Sep 8), tested against
// deep, real history (up to 120 real days): a whale buy only shows a
// genuine edge when the market is actually oversold at that exact
// moment, not blended across every condition. Long side requires a
// real outflow (large trade, closed up) landing on a genuinely
// oversold market, 57-58%, held consistently across two, separate,
// increasingly deep checks. Short side is the real, direct mirror,
// a large sell (inflow) landing on a genuinely overbought market,
// 55% on a real if more modest sample, still testing, not yet
// promoted. Both read the real RSI state from the same 1-minute
// candles the whale trade itself was detected on, not the slower,
// 4h basis this signal fires on, since that's genuinely what was
// tested. 4h only, the actual checkpoint the edge lives at.
function detectWhaleFlow(ctx) {
  const { tfKey, opts, bias, price, a, liveGate } = ctx;
  if (tfKey !== "4h") return null;
  if (opts.recentWhaleOutflowOversold) {
    const bc = biasContext(bias, "bull");
    const pc = provenContext("Whale Flow", tfKey, "bull", liveGate, ctx.regime, ctx.regimeGate);
    const strength = clamp(0.55 + bc.boost + pc.boost);
    const note = joinNote("A real, large buy just hit Coinbase, and the market was genuinely oversold at that exact moment. That combination is the real, honest edge, not the buy alone.", bc.phrase, pc.phrase);
    return { type: "whale", label: "Whale Flow", dir: "bull", strength, biasTag: bc.tag, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note, ...levels(price, a, "bull") };
  }
  if (opts.recentWhaleInflowOverbought) {
    const bc = biasContext(bias, "bear");
    const pc = provenContext("Whale Flow", tfKey, "bear", liveGate, ctx.regime, ctx.regimeGate);
    const strength = clamp(0.55 + bc.boost + pc.boost);
    const note = joinNote("A real, large sell just hit Coinbase, and the market was genuinely overbought at that exact moment. That combination is the real, honest edge, not the sell alone.", bc.phrase, pc.phrase);
    return { type: "whale", label: "Whale Flow", dir: "bear", strength, biasTag: bc.tag, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note, ...levels(price, a, "bear") };
  }
  return null;
}
