...
const GRIND_MIN_AGREE = 8;
const GRIND_MIN_MOVE_MULT = 3;
...
export function computeSignals(candles, tfKey, th, opts = {}) {
  ...
  // 30m and 1h are the confirming tier, checked directly against their
  // own candles. 5m and 15m are the fast, immediate arm, only ever fire
  // while a 30m-equivalent grind is actually confirmed in the same
  // direction, this isn't a second independent signal, it's the timing
  // layer of the same one. Zero backtest history yet, same starting
  // point every other signal here began at.
  if (tfKey === "30m" || tfKey === "1h") {
    const grind = detectGrind(candles);
    if (grind) {
      const label = grind.dir === "bull" ? "Grind Up" : "Grind Down";
      const bc = biasContext(bias, grind.dir);
      const pc = provenContext(label, tfKey, grind.dir);
      const strength = clamp(0.4 + (grind.agree - GRIND_MIN_AGREE) * 0.08 + bc.boost + pc.boost);
      const note = joinNote(`${grind.agree} of the last ${GRIND_WINDOW} bars ran the same direction, a real, sustained move, not a single sharp break.`, bc.phrase, pc.phrase);
      signals.push({ type: "grind", label, dir: grind.dir, strength, biasTag: bc.tag, tier: pc.tag, tierRate: pc.rate, note, ...levels(price, a, grind.dir) });
    }
  } else if (tfKey === "5m" || tfKey === "15m") {
