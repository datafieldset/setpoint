...
export function volatilityMeter(candles) {
  ...
  const exhausted = wasRecentlyHighVol && declining;
  ...
  // trendContext already used elsewhere in this file).
  ...
  return { score: Math.round(score), regime, label };
}
...
export function computeSignals(candles, tfKey, th, opts = {}) {
  ...
  if (volRatio >= th.volMult) {
    ...
    // exploit yet. Long side untouched here, that's a bigger, separate
    // collapse already tracked elsewhere, not what this specific gate targets.
  ...
  if (th.accumVolTrend) {
    ...
    if (closedEnd - 2 * w + 1 >= 0) {
      ...
      if (volTrend >= th.accumVolTrend && rangePct <= flatCeiling && tfKey !== "1h" && tfKey !== "15m") {
      ...
      // Quiet Build + Whale Confirmation (Aug 17): a genuinely new, separate
      // signal, not a change to the one above. Same real "Quiet accumulation"
      // condition, plus a real, recent whale outflow burst (last ~4h, the
      // exact checkpoint whale flow has actually proven strongest, 78% in
      // real backtests) as an additional confirmation. Only on 4h, its own
      // new timeframe, so the original 5m/30m Quiet Build is completely
      // untouched, still live, still verified, no risk to something that's
      // actually working. Zero backtest history, same starting point every
      // other signal here began at.
      if (tfKey === "4h" && volTrend >= th.accumVolTrend && rangePct <= flatCeiling && opts.recentWhaleOutflow) {
        const pickup = Math.round((volTrend - 1) * 100);
        const label = "Quiet Build + Whale";
        const pc = provenContext(label, tfKey, "bull");
        const note = joinNote(`Volume up ${pickup}% over the last ${w} bars while price held inside a ${rangePct.toFixed(2)}% range, and a real whale outflow burst confirmed it in the last 4 hours.`, pc.phrase);
        signals.push({ type: "accumulation", label, dir: "bull", strength: clamp((volTrend - th.accumVolTrend) / th.accumVolTrend + 0.15 + pc.boost), tier: pc.tag, tierRate: pc.rate, note, ...levels(price, a, "bull") });
      }
    }
  }
