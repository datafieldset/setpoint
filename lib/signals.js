...
// One real, unified "what stage are we in, and is it starting or ending"
// read, built entirely from logic already proven elsewhere in this file,
// not a new, separate idea. Real trend detection decides the stage when
// a genuine trend actually exists, the volatility meter's own ratio
// decides it otherwise, and that same meter's exhaustion read (now
// checking both real volatility decline and real momentum deceleration)
// decides whether the current stage looks like it's building or already
...
export function marketRegime(candles, tfKey) {
  ...
  const isTrending = trend.adx >= 20;
  let stage;
  if (isTrending) {
    stage = trend.plusDI > trend.minusDI ? "bullish-trending" : "bearish-trending";
  } else if (vol.regime === "expansion-strong" || vol.regime === "exhaustion") {
  ...
  const label = {
    "bullish-trending": "Bullish, trending",
    "bearish-trending": "Bearish, trending",
    "high-volatility": "High volatility",
    "low-volatility": "Low volatility",
    "sideways-ranging": "Sideways, ranging",
  }[stage];

  // "Ending" reuses the exact same exhaustion read the meter above already
  // computes, real volatility decline or real momentum deceleration,
  // whichever tripped it. Anything else reads as still building, the
  // honest default when nothing points to it running out of steam yet.
  const phase = vol.regime === "exhaustion" ? "ending" : "building";

  return { stage, label, phase, adx: Math.round(trend.adx) };
}
...
export function computeSignals(candles, tfKey, th, opts = {}) {
  ...
  if (tfKey === "4h" && opts.recentWhaleOutflow) {
    ...
    const note = joinNote("A real, large whale outflow burst just confirmed on Coinbase, the checkpoint this has actually proven strongest at.", bc.phrase, pc.phrase);
