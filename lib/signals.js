...
// not a proxy. Built by pooling two independent backtest runs per
// (label, timeframe, direction) combination. PROVEN = win rate >= 40% pooled.
// WEAK = win rate < 25% pooled, consistently bad in both runs separately, not
// just on average. Everything else stays untagged; most combinations simply
// don't have strong enough evidence yet to call good or bad, and pretending
// otherwise would be overfitting to two runs of one market stretch.
// Re-run /api/backtest periodically and update this table by hand, it is not
// pulled from a live database, so the live app has no database dependency.
const PROVEN_COMBOS = new Set([
  "RSI oversold|5m|bull", "EMA cross up|30m|bull", "EMA cross down|5m|bear",
  "RSI oversold|15m|bull", "RSI overbought|5m|bear",
]);
const WEAK_COMBOS = new Set([
...
function provenContext(label, tfKey, dir) {
  ...
  if (PROVEN_COMBOS.has(key)) return { tag: "proven", boost: 0.15, phrase: "this exact setup has backtested well twice" };
  if (WEAK_COMBOS.has(key)) return { tag: "weak", boost: -0.30, phrase: "this exact setup has backtested poorly twice" };
  return { tag: null, boost: 0, phrase: null };
}
...
export function computeSignals(candles, tfKey, th, opts = {}) {
  ...
  if (Math.abs(pct) >= th.pctMin) {
    ...
    const strength = clamp((Math.abs(pct) - th.pctMin) / th.pctMin + vc.boost + tc.boost + pc.boost);
    const note = `${pct > 0 ? "+" : ""}${pct.toFixed(2)}% in one ${TF[tfKey].label} bar`
      + (vc.phrase ? `, ${vc.phrase}` : "") + (tc.phrase ? `, ${tc.phrase}` : "") + (pc.phrase ? `, ${pc.phrase}` : "");
    signals.push({ type: "move", label: "Momentum", dir, strength, volTag: vc.tag, trendTag: tc.tag, tier: pc.tag, note, ...levels(price, a, dir) });
  }
  if (volRatio >= th.volMult) {
    const dir = last.close >= last.open ? "bull" : "bear";
    const tc = trendContext(trend, dir);
    const pc = provenContext("Volume spike", tfKey, dir);
    const strength = clamp((volRatio - th.volMult) / th.volMult + tc.boost + pc.boost);
    const note = `${volRatio.toFixed(1)}× the 20-bar average volume` + (tc.phrase ? `, ${tc.phrase}` : "") + (pc.phrase ? `, ${pc.phrase}` : "");
    signals.push({ type: "volume", label: "Volume spike", dir, strength, trendTag: tc.tag, tier: pc.tag, note, ...levels(price, a, dir) });
  }
  if (r != null && r <= th.rsiLow) {
    const vc = volumeContext(volRatio);
    const tc = trendContext(trend, "bull");
    const pc = provenContext("RSI oversold", tfKey, "bull");
    const strength = clamp((th.rsiLow - r) / th.rsiLow + vc.boost + tc.boost + pc.boost);
    const note = `RSI ${r.toFixed(0)}, oversold reading` + (vc.phrase ? `, ${vc.phrase}` : "") + (tc.phrase ? `, ${tc.phrase}` : "") + (pc.phrase ? `, ${pc.phrase}` : "");
    signals.push({ type: "rsi", label: "RSI oversold", dir: "bull", strength, volTag: vc.tag, trendTag: tc.tag, tier: pc.tag, note, ...levels(price, a, "bull") });
  } else if (r != null && r >= th.rsiHigh) {
    const vc = volumeContext(volRatio);
    const tc = trendContext(trend, "bear");
    const pc = provenContext("RSI overbought", tfKey, "bear");
    const strength = clamp((r - th.rsiHigh) / (100 - th.rsiHigh) + vc.boost + tc.boost + pc.boost);
    const note = `RSI ${r.toFixed(0)}, overbought reading` + (vc.phrase ? `, ${vc.phrase}` : "") + (tc.phrase ? `, ${tc.phrase}` : "") + (pc.phrase ? `, ${pc.phrase}` : "");
    signals.push({ type: "rsi", label: "RSI overbought", dir: "bear", strength, volTag: vc.tag, trendTag: tc.tag, tier: pc.tag, note, ...levels(price, a, "bear") });
  }
  if (e9.length && e21.length) {
    ...
    if (dPrev <= 0 && dNow > 0) {
      ...
      signals.push({ type: "cross", label: "EMA cross up", dir: "bull", strength: clamp(0.65 + vc.boost + tc.boost + pc.boost), volTag: vc.tag, trendTag: tc.tag, tier: pc.tag, note: "9 EMA crossed above 21 EMA" + (vc.phrase ? `, ${vc.phrase}` : "") + (tc.phrase ? `, ${tc.phrase}` : "") + (pc.phrase ? `, ${pc.phrase}` : ""), ...levels(price, a, "bull") });
    } else if (dPrev >= 0 && dNow < 0) {
      const tc = trendContext(trend, "bear");
      const pc = provenContext("EMA cross down", tfKey, "bear");
      signals.push({ type: "cross", label: "EMA cross down", dir: "bear", strength: clamp(0.65 + vc.boost + tc.boost + pc.boost), volTag: vc.tag, trendTag: tc.tag, tier: pc.tag, note: "9 EMA crossed below 21 EMA" + (vc.phrase ? `, ${vc.phrase}` : "") + (tc.phrase ? `, ${tc.phrase}` : "") + (pc.phrase ? `, ${pc.phrase}` : ""), ...levels(price, a, "bear") });
    }
  }
