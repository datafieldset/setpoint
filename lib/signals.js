...
export function computeSignals(candles, tfKey, th, opts = {}) {
  ...
  } else if (r != null && r >= th.rsiHigh && tfKey !== "5m") {
    // RSI overbought 5m bear removed entirely: confirmed 0% across
    // multiple separate backtest runs, real dead weight, not a fluke.
    const vc = volumeContext(volRatio);
  ...
  if (e9.length && e21.length) {
    ...
    } else if (dPrev >= 0 && dNow < 0 && tfKey !== "30m") {
      // EMA cross down 30m bear removed entirely: confirmed 0% across
      // multiple separate backtest runs, real dead weight, not a fluke.
      const tc = trendContext(trend, "bear");
  ...
  if (fraction > 0.15 && fraction < 0.85 && th.paceMult) {
    ...
    if (paceRatio >= th.paceMult) {
      const dir = last.close >= last.open ? "bull" : "bear";
      // Volume building early, bear side on 5m/15m removed entirely:
      // confirmed 0% repeatedly, same pattern every time it's been checked.
      if (!(dir === "bear" && (tfKey === "5m" || tfKey === "15m"))) {
  ...
  if (th.accumVolTrend) {
    ...
    if (closedEnd - 2 * w + 1 >= 0) {
      ...
      // Quiet accumulation 1h bull removed entirely: 11% backtest, 20%
      // live on a real 20-fire sample, consistently weak, never close.
      if (volTrend >= th.accumVolTrend && rangePct <= flatCeiling && tfKey !== "1h") {
