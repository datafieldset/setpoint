...
export function computeSignals(candles, tfKey, th, opts = {}) {
  ...
  if ((bias && bias.dir && risk && risk.level !== "low") || ownStretched) {
    ...
    if (confirm.score >= 1.5) {
      ...
      signals.push({ type: "reversal", label: "Reversal watch", dir, strength, volTag: vc.tag, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, confirmTier, note, ...levels(price, a, dir) });
    }
  }
  ...
  // record — see the real, named exception in the cron and client
  // logging logic.
  {
    ...
    if (n >= baselineWindow + squeezeWindow + 1) {
      const ranges = [];
      ...
      if (genuinelyTight) {
        ...
        if (breakUp || breakDown) {
          ...
          signals.push({ type: "coil", label: "Coil", dir, strength: 0.6, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note, ...levels(price, a, dir) });
        }
      }
    }
  }
  ...
  // real, temporary exception as Coil.
  {
    ...
    if (swings.length >= 2) {
      ...
      if (!stillForming && lastSwing.type === "LH" && priorSwing.kind === "low") {
        ...
        if (brokeJustNow) {
          ...
          signals.push({ type: "swing", label: "Swing", dir, strength: 0.6, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note, ...levels(price, a, dir) });
        }
      } else if (!stillForming && lastSwing.type === "HL" && priorSwing.kind === "high") {
        const level = priorSwing.price;
        const brokeJustNow = price > level && closes[n - 2] <= level;
        if (brokeJustNow) {
          const dir = "bull";
  ...
  if (Math.abs(pct) >= th.pctMin) {
    ...
    signals.push({ type: "move", label: "Momentum", dir, strength, volTag: vc.tag, trendTag: tc.tag, biasTag: bc.tag, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note, ...levels(price, a, dir) });
  }
  if (volRatio >= th.volMult) {
    ...
    if (!shortGateBlocks && !longGateBlocks && !shortBiasGateBlocks) {
      const pc = provenContext("Volume spike", tfKey, dir, liveGate);
      const bc = biasContext(bias, dir);
      ...
      signals.push({ type: "volume", label: "Volume spike", dir, strength, trendTag: tc.tag, biasTag: bc.tag, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note, ...levels(price, a, dir) });
    }
  }
  ...
  if (r != null && r <= th.rsiLow && tfKey !== "30m") {
    ...
    if (tc.tag !== "against") {
      ...
      signals.push({ type: "rsi", label: "RSI oversold", dir: "bull", strength, volTag: vc.tag, trendTag: tc.tag, biasTag: bc.tag, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note, ...levels(price, a, "bull") });
    }
    ...
    if (vc.tag !== "confirmed" && tc.tag !== "against") {
      ...
      signals.push({ type: "rsi", label: "RSI overbought", dir: "bear", strength, volTag: vc.tag, trendTag: tc.tag, biasTag: bc.tag, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note, ...levels(price, a, "bear") });
    }
  }
  if (e9.length && e21.length) {
    ...
    if (false && dPrev <= 0 && dNow > 0) {
      const tc = trendContext(trend, "bull");
      const pc = provenContext("EMA cross up", tfKey, "bull", liveGate);
      const bc = biasContext(bias, "bull");
      signals.push({ type: "cross", label: "EMA cross up", dir: "bull", strength: clamp(0.65 + vc.boost + tc.boost + pc.boost + bc.boost), volTag: vc.tag, trendTag: tc.tag, biasTag: bc.tag, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note: joinNote("9 EMA crossed above 21 EMA.", vc.phrase, tc.phrase, bc.phrase, pc.phrase), ...levels(price, a, "bull") });
    } else if (false && dPrev >= 0 && dNow < 0) {
      const tc = trendContext(trend, "bear");
      const pc = provenContext("EMA cross down", tfKey, "bear", liveGate);
      const bc = biasContext(bias, "bear");
      signals.push({ type: "cross", label: "EMA cross down", dir: "bear", strength: clamp(0.65 + vc.boost + tc.boost + pc.boost + bc.boost), volTag: vc.tag, trendTag: tc.tag, biasTag: bc.tag, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note: joinNote("9 EMA crossed below 21 EMA.", vc.phrase, tc.phrase, bc.phrase, pc.phrase), ...levels(price, a, "bear") });
    }
  }
  ...
  if (fraction > 0.15 && fraction < 0.85 && th.paceMult) {
    ...
    if (paceRatio >= th.paceMult) {
      ...
      if (dir !== "bear") {
        ...
        signals.push({ type: "pace", label: "Volume building early", dir, strength: clamp((paceRatio - th.paceMult) / th.paceMult + pc.boost), tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note: joinNote(`Volume already ${paceRatio.toFixed(1)}x the usual pace with ${pctLeft}% of the bar left.`, pc.phrase), ...levels(price, a, dir) });
      }
    }
  }
  ...
  if (th.accumVolTrend) {
    ...
    if (closedEnd - 2 * w + 1 >= 0) {
      ...
      if (volTrend >= th.accumVolTrend && rangePct <= flatCeiling && tfKey !== "1h" && tfKey !== "15m" && trend.adx < 20) {
        ...
        signals.push({ type: "accumulation", label: "Quiet accumulation", dir: "bull", strength: clamp((volTrend - th.accumVolTrend) / th.accumVolTrend + pc.boost), tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note: joinNote(`Volume up ${pickup}% over the last ${w} bars while price held inside a ${rangePct.toFixed(2)}% range.`, pc.phrase), ...levels(price, a, "bull") });
      }
    }
  }
  ...
  if (transition) {
    ...
    signals.push({ type: "volatility", label, dir: transition.dir, strength, biasTag: bc.tag, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note, ...levels(price, a, transition.dir) });
  }
  ...
  if (tfKey === "5m" || tfKey === "15m") {
    ...
    if (fast) {
      ...
      signals.push({ type: "grind", label, dir: fast.dir, strength, biasTag: bc.tag, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note, ...levels(price, a, fast.dir) });
    }
  }
  ...
  if (tfKey === "4h" && opts.recentWhaleOutflow) {
    ...
    signals.push({ type: "whale", label: "Whale Flow", dir: "bull", strength, biasTag: bc.tag, tier: pc.tag, tierRate: pc.rate, tierIsLive: pc.isLive, note, ...levels(price, a, "bull") });
  }
