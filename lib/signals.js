...
export function computeSignals(candles, tfKey, th, opts = {}) {
  ...
  if (Math.abs(pct) >= th.pctMin) {
    ...
    const note = joinNote(`${pct > 0 ? "+" : ""}${pct.toFixed(2)}% in one ${TF[tfKey].label} bar.`, vc.phrase, tc.phrase, pc.phrase);
  ...
  if (volRatio >= th.volMult) {
    ...
    const note = joinNote(`${volRatio.toFixed(1)}× the 20-bar average volume.`, tc.phrase, pc.phrase);
  ...
  if (r != null && r <= th.rsiLow) {
    ...
    const note = joinNote(`RSI ${r.toFixed(0)}, oversold reading.`, vc.phrase, tc.phrase, pc.phrase);
    ...
    const note = joinNote(`RSI ${r.toFixed(0)}, overbought reading.`, vc.phrase, tc.phrase, pc.phrase);
  ...
  if (e9.length && e21.length) {
    ...
    if (dPrev <= 0 && dNow > 0) {
      ...
      signals.push({ type: "cross", label: "EMA cross up", dir: "bull", strength: clamp(0.65 + vc.boost + tc.boost + pc.boost), volTag: vc.tag, trendTag: tc.tag, tier: pc.tag, note: joinNote("9 EMA crossed above 21 EMA.", vc.phrase, tc.phrase, pc.phrase), ...levels(price, a, "bull") });
    } else if (dPrev >= 0 && dNow < 0) {
      const tc = trendContext(trend, "bear");
      const pc = provenContext("EMA cross down", tfKey, "bear");
      signals.push({ type: "cross", label: "EMA cross down", dir: "bear", strength: clamp(0.65 + vc.boost + tc.boost + pc.boost), volTag: vc.tag, trendTag: tc.tag, tier: pc.tag, note: joinNote("9 EMA crossed below 21 EMA.", vc.phrase, tc.phrase, pc.phrase), ...levels(price, a, "bear") });
    }
  }
