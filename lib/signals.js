...
// market bias layer exists to catch. Everything else stays untagged unless
// it clears its own bar; most combinations simply don't have strong enough
// evidence yet to call good or bad, and pretending otherwise would be
// overfitting to a handful of runs over one market stretch.
...
const PROVEN_COMBOS = new Set([
  "EMA cross up|30m|bull", "EMA cross down|5m|bear",
...
export function computeSignals(candles, tfKey, th, opts = {}) {
  ...
  if (bias && bias.dir && risk && risk.level !== "low") {
    ...
    const base = risk.level === "high" ? 0.55 : 0.35;
