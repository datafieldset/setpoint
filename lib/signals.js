...
export function provenContext(label, tfKey, dir, liveGate) {
  const key = `${label}|${TF[tfKey]?.label || tfKey}|${dir}`;
  const entry = SIGNAL_RATES[key];
  if (!entry || entry.rate == null) return { tag: null, boost: 0, phrase: null, rate: null, isLive: false };
  ...
  const staticProven = entry.rate >= PROVEN_THRESHOLD;
  ...
  if (rate >= PROVEN_THRESHOLD) {
    return { tag: "proven", boost: 0.15, phrase: `This setup ${useLive ? "is" : "has backtested at"} ${Math.round(rate * 100)}%.`, rate, isLive: useLive };
  }
  return { tag: "tested", boost: -0.30, phrase: `This setup ${useLive ? "is" : "has backtested at"} ${Math.round(rate * 100)}%.`, rate, isLive: useLive };
}
...
function buildSignalContext(candles, tfKey, th, opts) {
  ...
  return { candles, tfKey, th, opts, n, now, liveGate, closes, highs, lows, vols, price, pct, volAvg, volRatio, r, e9, e21, a, trend, bias, risk, last };
}
...
function detectReversalWatch(ctx) {
  ...
  const pc = provenContext("Reversal watch", tfKey, dir, liveGate);
...
function detectCoil(ctx) {
  ...
  const pc = provenContext("Coil", tfKey, dir, liveGate);
...
function detectSwing(ctx) {
  ...
  if (lastSwing.type === "LH" && priorSwing.kind === "low") {
    ...
    const dir = "bear";
    const pc = provenContext("Swing", tfKey, dir, liveGate);
...
function detectSwingEarly(ctx) {
  ...
  if (lastSwing.type === "LH") {
    const dir = "bear";
    const pc = provenContext("Swing Early", tfKey, dir, liveGate);
...
function detectMomentum(ctx) {
  ...
  const pc = provenContext("Momentum", tfKey, dir, liveGate);
...
function detectVolumeSpike(ctx) {
  ...
  const pc = provenContext("Volume spike", tfKey, dir, ctx.liveGate);
  const bc = biasContext(bias, dir);
...
function detectRsiOversoldOverbought(ctx) {
  ...
  if (r <= th.rsiLow && tfKey !== "30m") {
    ...
    const pc = provenContext("RSI oversold", tfKey, "bull", liveGate);
  ...
  if (r >= th.rsiHigh && tfKey === "30m") {
    ...
    const pc = provenContext("RSI overbought", tfKey, "bear", liveGate);
...
function detectVolumeBuildingEarly(ctx) {
  ...
  const pc = provenContext("Volume building early", tfKey, dir, liveGate);
...
function detectQuietAccumulation(ctx) {
  ...
  const pc = provenContext("Quiet accumulation", tfKey, "bull", liveGate);
...
function detectWhaleFlow(ctx) {
  ...
  if (opts.recentWhaleOutflowOversold) {
    const bc = biasContext(bias, "bull");
    const pc = provenContext("Whale Flow", tfKey, "bull", liveGate);
  ...
  if (opts.recentWhaleInflowOverbought) {
    const bc = biasContext(bias, "bear");
    const pc = provenContext("Whale Flow", tfKey, "bear", liveGate);
    const strength = clamp(0.55 + bc.boost + pc.boost);
