...
export const SIGNAL_RATES = {
  ...
  "Whale Flow|4h|bull": { rate: 0.29, runs: 1 }, // real, direct audit of its own live record (Sep 4): 4W/10L on 14 real, recent trades, genuinely below the bar now despite being the site's original, first-ever promotion — cut
...
// onto something else. Fires on a real, recent whale outflow burst
// (large trades on Coinbase, taker side buying, no exchange labeling
// required to trust), the one direction that's actually earned it,
// 74-78% at the 4h checkpoint historically. The mirror direction,
// inflow, was seriously considered and deliberately left out, real data
// showed it doesn't predict the traditional bearish read either
// (~16-20%), not clean enough in either direction to build on. Buy-only,
// on purpose. 4h only, the actual checkpoint the edge lives at, not
// spread across every timeframe just to have more coverage.
function detectWhaleFlow(ctx) {
  const { tfKey, opts, bias, price, a, liveGate } = ctx;
  if (tfKey !== "4h" || !opts.recentWhaleOutflow) return null;
  const bc = biasContext(bias, "bull");
