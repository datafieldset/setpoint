...
export function computeSignals(candles, tfKey, th, opts = {}) {
  ...
  if (Math.abs(pct) >= th.pctMin) {
    ...
    const strength = clamp((Math.abs(pct) - th.pctMin) / th.pctMin + vc.boost + tc.boost);
    const note = `${pct > 0 ? "+" : ""}${pct.toFixed(2)}% in one ${TF[tfKey].label} bar`
      + (vc.phrase ? `, ${vc.phrase}` : "") + (tc.phrase ? `, ${tc.phrase}` : "");
    signals.push({ type: "move", label: "Momentum", dir, strength, volTag: vc.tag, trendTag: tc.tag, note, ...levels(price, a, dir) });
  }
  if (volRatio >= th.volMult) {
    const dir = last.close >= last.open ? "bull" : "bear";
    const tc = trendContext(trend, dir);
    const strength = clamp((volRatio - th.volMult) / th.volMult + tc.boost);
    const note = `${volRatio.toFixed(1)}× the 20-bar average volume` + (tc.phrase ? `, ${tc.phrase}` : "");
    signals.push({ type: "volume", label: "Volume spike", dir, strength, trendTag: tc.tag, note, ...levels(price, a, dir) });
  }
  if (r != null && r <= th.rsiLow) {
    const vc = volumeContext(volRatio);
    const tc = trendContext(trend, "bull");
    const strength = clamp((th.rsiLow - r) / th.rsiLow + vc.boost + tc.boost);
    const note = `RSI ${r.toFixed(0)}, oversold reading` + (vc.phrase ? `, ${vc.phrase}` : "") + (tc.phrase ? `, ${tc.phrase}` : "");
    signals.push({ type: "rsi", label: "RSI oversold", dir: "bull", strength, volTag: vc.tag, trendTag: tc.tag, note, ...levels(price, a, "bull") });
  } else if (r != null && r >= th.rsiHigh) {
    const vc = volumeContext(volRatio);
    const tc = trendContext(trend, "bear");
    const strength = clamp((r - th.rsiHigh) / (100 - th.rsiHigh) + vc.boost + tc.boost);
    const note = `RSI ${r.toFixed(0)}, overbought reading` + (vc.phrase ? `, ${vc.phrase}` : "") + (tc.phrase ? `, ${tc.phrase}` : "");
    signals.push({ type: "rsi", label: "RSI overbought", dir: "bear", strength, volTag: vc.tag, trendTag: tc.tag, note, ...levels(price, a, "bear") });
  }
  if (e9.length && e21.length) {
    ...
    if (dPrev <= 0 && dNow > 0) {
      const tc = trendContext(trend, "bull");
      signals.push({ type: "cross", label: "EMA cross up", dir: "bull", strength: clamp(0.65 + vc.boost + tc.boost), volTag: vc.tag, trendTag: tc.tag, note: "9 EMA crossed above 21 EMA" + (vc.phrase ? `, ${vc.phrase}` : "") + (tc.phrase ? `, ${tc.phrase}` : ""), ...levels(price, a, "bull") });
    } else if (dPrev >= 0 && dNow < 0) {
      const tc = trendContext(trend, "bear");
      signals.push({ type: "cross", label: "EMA cross down", dir: "bear", strength: clamp(0.65 + vc.boost + tc.boost), volTag: vc.tag, trendTag: tc.tag, note: "9 EMA crossed below 21 EMA" + (vc.phrase ? `, ${vc.phrase}` : "") + (tc.phrase ? `, ${tc.phrase}` : ""), ...levels(price, a, "bear") });
    }
  }
